...
//run 
//root -l 'FinalplotPFDijetDMV_qq_1D_limit_vs_width.C("FinalPlotDM_V")'
const double PI=3.14159265;
void FinalplotPFDijetDMV_qq_1D_limit_vs_width(string outputDIR) {
  ...
  GammaOverM_prime = ((18*narrowLimitg_qPrime[i]*narrowLimitg_qPrime[i]) + 1)/(12*PI);
...
int zMass=0; double obs, expPlus2,expPlus,exp,expMinus,expMinus2=0;
  while (!input_1.eof()) {
    ...
    if (!input_1.fail()) {
      ...
      grexp->SetPoint(expcounter, double(zMass), exp);
      expcounter++;
      // find max and min for frame
      medMassList.push_back(zMass);
      grobs->SetPoint(obscounter, double(zMass),obs);
      obscounter++;
      GammaOverM=0;
      GammaOverM =((18*exp*exp) + 1)/(12*PI);
      cout <<"mass = "<<zMass<< " exp = "<<exp<<" obs = "<<obs<<endl;
    // 1 sigma dw
      grexp_1sigma_dw->SetPoint(exp_down_counter_1s, double(zMass),expMinus);      
      exp_down_counter_1s++;
    // 1 sigma up
      grexp_1sigma_up->SetPoint(exp_up_counter_1s, double(zMass),expPlus);      
      exp_up_counter_1s++;

    // 2 sigma dw
      grexp_2sigma_dw->SetPoint(exp_down_counter_2s, double(zMass),expMinus2);      
      exp_down_counter_2s++;
    // 2 sigma up
      grexp_2sigma_up->SetPoint(exp_up_counter_2s, double(zMass),expPlus2);      
      exp_up_counter_2s++;
    }
  }
  ...
  frame->Draw();
  CMS_lumi(canvas,"36",false,true,false,0.0,0.0);
...
//  line->Draw("L same");
  ...
  gPad->Modified(); 
  gPad->Update();
  canvas->SaveAs((outputDIR+"/Final_PFDMV-g_q-limit_observed_35p9fb_final_1D_M"+to_string(int(1))+".pdf").c_str(),"pdf");
  canvas->SaveAs((outputDIR+"/Final_PFDMV-g_q-limit_observed_35p9fb_final_1D_M"+to_string(int(1))+".png").c_str(),"pdf");
  canvas->SaveAs((outputDIR+"/Final_PFDMV-g_q-limit_observed_35p9fb_final_1D_M"+to_string(int(1))+".C").c_str(),"pdf");

  if(saveOutputFile){
    TFile* outputFile = new TFile((outputDIR+"/grFinal_PFDMV-g_q-withoutNarrow-limit_35p9fb.root").c_str(),"RECREATE");
    outputFile->cd();
    ...
    grobsNarrowg_q->Write("graph_observedNarrow");
    mg->Write("graph_All");
    leg->SetName("My_leg");
    leg->Write();
    outputFile->Write();
